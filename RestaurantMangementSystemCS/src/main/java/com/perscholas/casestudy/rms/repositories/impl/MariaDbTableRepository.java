...
public class MariaDbTableRepository implements TableRepository {
	...
	public List<Table> getAll() throws SQLException {
		String selectGetAll = "SELECT * FROM `table`;";
		List<Table> result = mariaDbJdbcTemplate.query(selectGetAll, new TableMapper());
		return result;
	}
	...
	public Table getById(Integer tableId, Integer userId) throws ClassNotFoundException, IOException, SQLException {
		String selectById = "SELECT * FROM `table` WHERE `userId` = :userId AND `tableId` = :tableId;";
		...
		try {
			table = (Table) mariaDbJdbcTemplate.queryForObject(selectById, params, new TableMapper());
